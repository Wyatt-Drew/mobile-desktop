...
//sendMessage
  ...
  const sendSubjectId = () => {
    if (ws && ws.readyState === WebSocket.OPEN) {
      if (subjectId) {
        sendMessage("subjectId", subjectId); // Reuse the sendMessage utility
        setCurrentScreen(SCREENS.WELCOME); // Transition to the WELCOME screen
      } else {
        console.error("Subject ID is missing. Unable to send.");
      }
    } else {
